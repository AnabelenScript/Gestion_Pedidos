...
function serviceUrl(value: string, key: string): string {
  let url: URL;
  try {
    url = new URL(value);
  } catch {
  ...
  if (!['http:', 'https:'].includes(url.protocol)) {
    throw new Error(`La variable ${key} debe usar HTTP o HTTPS`);
  }
...
export function validateEnvironment(config: Record<string, unknown>) {
  ...
  if ((validated.JWT_SECRET as string).length < 32) {
  ...
  validated.INVENTORY_URL = serviceUrl(
    validated.INVENTORY_URL as string,
    'INVENTORY_URL',
  );
  validated.PAYMENTS_URL = serviceUrl(
    validated.PAYMENTS_URL as string,
    'PAYMENTS_URL',
  );
  validated.PORT = integer(config.PORT, 'PORT', 3000);
  validated.DATABASE_PORT = integer(
    config.DATABASE_PORT,
    'DATABASE_PORT',
    5432,
  );
  validated.REDIS_PORT = integer(config.REDIS_PORT, 'REDIS_PORT', 6379);
